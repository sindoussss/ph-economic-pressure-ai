...
The Monitor is hybrid: it pulls fresh Reddit + Google Trends into the EXPLORATORY
social snapshot when the optional deps (pytrends/praw), Reddit creds, and network
are available, and otherwise silently leaves the frozen snapshot in place. This
never raises and never blocks the run for long.
...
def refresh_social_snapshot(corpus_dir: Path = CORPUS_DIR) -> dict:
    ...
    if _have('praw'):
        try:
            from ph_economic_ai.tools.refresh_social import refresh_reddit
