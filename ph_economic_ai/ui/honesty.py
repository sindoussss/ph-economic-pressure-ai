...
def announced_adjustment_line(announcement, product: str = 'gasoline',
    ...
    not: the oil companies filed it with DOE and it takes effect whether or not
    this app exists. Presenting the two in the same voice would let a reader
    credit the app with the certainty of the announcement, which is the same
    borrowed-authority mistake `RSK-023` cost three withdrawn grades for.

    So the sentence names the source, names the week, and says plainly that it
    is not a forecast. Returns '' when nothing is announced for the period --
    ...
    if value is None:
        basis = announcement.get('basis')
        if basis == 'no_consensus':
            return ('DOE notice published, but the oil companies did not file a '
                    'single common change this week, so there is no announced '
                    'number to show.')
        return ''
    start, end = announcement.get('week_start'), announcement.get('week_end')
    when = f'{start} to {end}' if start and end else 'this week'
    n = announcement.get('n_companies')
