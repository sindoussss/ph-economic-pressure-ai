...
- Final Outputs / Validated Accuracy: migrate onto `theme.card()` (already used elsewhere) with `theme.eyebrow`/`theme.muted`/`theme.hairline` throughout. **As implemented (Task 9 of the plan), this ended up as two specific substitutions in `_build_right`** -- the metrics-grid value color and the validated-accuracy summary-line labels -- not a sweep of the file's other stray `setStyleSheet` calls; see §6's note on remaining scope.
...
2. `stage4_report.py`'s sector-forecast cards, swarm and classic consensus panels, regional verdicts, final-outputs metrics grid, and validated-accuracy card migrated onto the editorial tokens (the specific sections this plan's tasks named) — **not** a full-file migration; see the note below.
...
**Acknowledged gap:** `stage4_report.py` still carries 42 `setStyleSheet` calls outside the sections above -- the top status bar, the export button, the recall banner, `_physics_anchor_label`, `_explanation_block`, classic-mode per-response cards (`name_lbl`/`est_lbl`/`stmt_lbl`), the dissenting-regions label, and the map basis/unvalidatable captions all still use raw hex. None of that was in scope for this plan's tasks; it's a known follow-up, not something this branch silently finished.

## 6. Why it matters

Finishes the specific sections this plan's own tasks named on the Report screen -- sector-forecast cards, both consensus panels, regional verdicts, the final-outputs grid, the validated-accuracy card, and `BSPAlertBanner` -- and removes a specific, user-identified credibility problem: an alert-box pattern that reads as generated rather than authored, on the app's most-seen screen, competing with the actual goal of looking like a rigorous analytical tool. It does not close out SP2d-1's full-file migration of `stage4_report.py`; see the acknowledged gap above. Confidence communication itself doesn't change — only whether the touched sections look like someone designed them on purpose.
