...
- `test_monitor.py` (confirmed: the existing `PressureMonitorPanel` coverage lives here, not in a `test_pressure_monitor.py` that doesn't exist): mixed-sign regression test on the breakdown row (assert `'+-'` never appears, assert a missing category renders as `—` not `0.0%`), same shape as this session's `test_set_alert_sector_breakdown_has_one_sign_not_two`. No existing test currently exercises `_sector_card` directly -- this is new coverage, not an extension of a prior test.
