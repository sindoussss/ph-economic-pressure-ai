...
> **Resolved 2026-08-20, and the question is now easier to answer.** This notice used to warn that the fuel row below said "efficient" while the committed artifact said `predictable` at +12.1% Ridge, DM p = 0.0337. **The artifact now says efficient too, so there is no divergence to defend.** The audit had been assigning `predictable` on a bare `dm_p < 0.05`, even though `verdict_from_panel` takes the BEST of four candidate methods and judges that maximum, so it must pay for k = 4; the repository's own multiple-testing artifact already listed `fuel_audit` under `survive_neither` at bonferroni_p 0.7104. PR #55 corrected the rule. **If asked "is the map really uniform, then?": yes, and the artifacts agree with the table rather than needing a caveat.** The underlying exploratory finding is unchanged and still disclosed: the Ridge edge is real in sample, survives neither Bonferroni nor Benjamini-Hochberg, was never predeclared, and is registered as `CLM-FUEL-EXPLORATORY-001`. Since 2026-08-20 it also has a preregistered confirmatory protocol (`docs/preregistration/2026-08-20-fuel-audit-confirmatory.md`) which records that confirming it needs roughly ninety-five monthly predictions against the seventeen available, so it stays exploratory by design. Checked by `python -m ph_economic_ai.benchmark.manuscript_check`, which reports `consistent` across all four documents.
