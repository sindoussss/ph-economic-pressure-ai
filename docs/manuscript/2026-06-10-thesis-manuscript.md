...
Because an anchor is a quantitative claim, it is itself testable against the same real series the audit uses, and is regressed there rather than asserted (`anchor_validation.json`; `tools/anchor_backtest.py`). The fuel anchor is validated as a genuine model of the *contemporaneous* pass-through: over 78 months of World Bank RON95 against monthly Brent and USD/PHP, its predicted monthly pump change correlates 0.60 with the realized change, matches direction 74% of the time, and yields a lower mean absolute error (₱2.21) than a no-change baseline (₱2.64). The ordinary-least-squares slope of realized on predicted change, 0.79, is a finding in its own right — Philippine pump prices pass through only about four-fifths of a monthly crude move, consistent with subsidy buffers, the weekly-averaging lag, and the partial, asymmetric adjustment the rockets-and-feathers literature documents (§2.7) — and is fed back as the anchor's calibration coefficient. This is a *contemporaneous* pass-through relationship, and does not contradict the one-month-ahead efficiency of §5.1: the anchor scales a *known* shock, it does not forecast one.

The other two anchors are reported with the same candour that governs the audit. Regressed against 175 months of PSA electricity CPI, the fuel-price anchor does *not* predict the monthly move (correlation 0.03–0.13 across lags); the robust electricity edge of §5.7 is recoverable only through the full generation-charge formula, which raw commodity prices proxy poorly. Against 172 months of PSA food CPI, persistence and an oil driver are both weak and statistically indistinguishable (correlation 0.18 vs 0.21, within one standard error at this sample), and a simple mean is competitive. Neither anchor is a predictor at monthly resolution. What each *is* — and what the anchor is for — is a magnitude guard: the ratio of the anchor's typical size to the realized monthly move is ≈1.0 for electricity and ≈0.9 for food, so each keeps a weak model's estimate correctly scaled even where it cannot forecast. Measured against the realized series, reconciliation more than halves the error of a simulated hallucinating model (₱3.79 → ₱1.58, a 58% reduction), and a robustness sweep over 10,357 scenarios and adversarial inputs finds no case in which the anchoring returns a non-finite or unbounded value.
...
### 6.7 Limitations
Monthly resolution; an RBOB fuel proxy with disclosed bias (r = 0.91, −₱5.88/L); modest samples (n = 38–143); conformal coverage that is approximate at small n (and here conservative); and a CPI series via IMF/DBnomics rather than the PSA microdata. The application's LLM/agent "swarm" is an interface and explanation layer, not a validated predictor — its agent-agreement numbers are labelled as such, distinct from the calibrated intervals. The anchoring layer (§6.6) is bounded in the same spirit: its fuel anchor is a contemporaneous pass-through model, not a forecast; its electricity and food anchors are magnitude guards that do not predict at monthly resolution; and its calibration coefficient is fit to a single 2017–2025 window and may drift.
