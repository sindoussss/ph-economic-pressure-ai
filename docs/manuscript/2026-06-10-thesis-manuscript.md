...
| ρ | n = 61, pool without mean | n = 151, pool without mean | either n, pool **with** mean |
|---|---|---|---|
| 0.00 | 43.3% | **99.7%** | 0.0% |
| 0.20 | 19.7% | 87.7% | 0.0% |
| 0.35 | 4.7% | 26.3% | 0.0% |
...
**What supports it.** It is not the baseline artifact of §4.7: fuel's lag-1 autocorrelation is ρ = +0.9475, far above the ½ crossover, and the closed form predicts the mean should score −2.087 against the random walk here against a measured −2.263. Against 400 surrogates built by circularly rotating each driver column — destroying alignment while preserving each driver's own autocorrelation, scale and trend — the observed skill has empirical p = 0.0000 (null mean −0.152, null maximum −0.031). It also survives selection: with the model and the naive both fixed on the first 70% of rows and scored only on the remaining 29 predictions, skill moves +0.1233 → +0.1187, a shrinkage of 0.005 (DM p = 0.0296), where headline MoM on the same protocol collapses +0.0329 → −0.0057. §5.8.1 reports this protocol (§4.9.1) run against all twenty-three of the manuscript's selection-holdout targets, not only these two. The mechanism is an interaction, not a single driver: own-lag alone scores −0.030 and drivers alone −0.118, and only the combination beats the random walk, which is error correction toward the RBOB/FX-implied level and is consistent with the measured pass-through (β_total = 0.596, R² = 0.490).

**Why it is not claimed.** The audit family is three targets, so Bonferroni gives α = 0.0167 against an observed 0.0337 in sample and 0.0296 on the holdout. The holdout carries only 29 predictions. Most importantly it was never predeclared: it emerged from a data correction, which is precisely the pattern the size study of §5.10 exists to distrust. It is registered as `CLM-FUEL-EXPLORATORY-001` and is a candidate for preregistered confirmation, not a result. That confirmation is now specified rather than deferred: `docs/preregistration/2026-08-20-fuel-audit-confirmatory.md` freezes the ridge specification and names the confirmatory window as every month after 2025-03, which no test in this thesis has used. Because a pre-declared single specification selects nothing, it applies α = 0.05 undivided rather than the threshold above. It also records why that confirmation is not reported here: scaling the observed |t| = 2.17 at seventy-two predictions, a one-sided test at 80% power needs roughly ninety-five monthly predictions against the seventeen the window currently holds, so the claim stays exploratory in this thesis by design and not by omission.
...
| Driver-only ablation, full sample (n = 204) | no_better_than_naive | mean | 0.0 | — |
| Driver-only ablation, robust (drop 6 preliminary months, n = 198) | no_better_than_naive | mean | 0.0 | — |
...
Second, the food-commodity **driver edge remains a clean null**, and is now null by a wider margin: driver-only Ridge (0.702) is *worse* than the constant (0.689), a skill of **−1.9%** (p = 0.72). The verdict is stable at both n = 204 and n = 198 (`driver_edge_robust = False`). Global commodity prices carry no within-month signal for Philippine food inflation, consistent with its strongly-local composition (fish, vegetables, import-controlled rice). This conclusion is unchanged by the correction — it was a null before and is a null now, which is exactly the kind of result that should be insensitive to the baseline pool.

### 5.7 Electricity: the flagship edge does not survive the mean

Electricity was the audit's strongest apparent positive, and it is the one the correction overturns most sharply. Using the PSA `04.5.1 - Electricity` gold and free energy predictors (Brent, natural gas, FX), the driver-only nowcast (n = 204) gave the headline result of the earlier draft: Ridge **+28.3% over the best naive, DM p = 0.0011**, stable across sub-samples (≤2023-12 +26%, p = 0.006; 2007–2016 +30%, p = 0.020; 2016–2026 +29%, p = 0.035) and surviving the trailing-preliminary robustness check. Every check the audit had *was* passed.

The baseline pool was the check it lacked. The RMSEs make the situation unambiguous:

| Method | RMSE (driver-only, n = 204) |
|---|---|
| **mean** | **2.3515** |
| Ridge | 2.3936 |
| ARIMA | 2.4925 |
...
### 5.8.1 Selection-honest re-test: all twenty-three targets

§4.9.1 describes the protocol. It reached its current coverage in three pre-registered stages: two targets (fuel, headline MoM short-sample) were run first; the remaining nine headline verdicts in §5.8's map were pre-registered on 2026-08-08, with the expectation that all nine would return `not_confirmed_on_holdout`, matching their published null or withdrawn status; and twelve PSA food sub-category frames — rice, meat, fish, dairy & eggs, vegetables and sugar, each in full-nowcast and driver-only form — were pre-registered in [`2026-08-12-food-subcategory-selection-holdout.md`](../preregistration/2026-08-12-food-subcategory-selection-holdout.md). `selection_holdout.json` reports all twenty-three, and this table is the whole of it — the same rows, and the same count, as the multiplicity family in §5.8:
...
| Rice MoM, full nowcast | 228 | 160 | 68 | +2.7% | +9.3% | −0.067 | 0.386 | not_confirmed_on_holdout |
| Rice MoM, driver-only | 228 | 160 | 68 | −47.7% | +7.3% | −0.550 | 0.777 | not_confirmed_on_holdout |
| Meat MoM, full nowcast | 228 | 160 | 68 | +9.4% | +10.5% | −0.012 | 0.119 | not_confirmed_on_holdout |
| Meat MoM, driver-only | 228 | 160 | 68 | −9.1% | −2.8% | −0.063 | 0.378 | not_confirmed_on_holdout |
| Fish MoM, full nowcast | 228 | 160 | 68 | −2.9% | +0.6% | −0.035 | 0.961 | not_confirmed_on_holdout |
| Fish MoM, driver-only | 228 | 160 | 68 | −11.4% | −3.5% | −0.080 | 0.786 | not_confirmed_on_holdout |
| Dairy & eggs MoM, full nowcast | 228 | 160 | 68 | +16.2% | −0.5% | 0.167 | 0.942 | not_confirmed_on_holdout |
| Dairy & eggs MoM, driver-only | 228 | 160 | 68 | −2.0% | −35.0% | 0.330 | 0.0201 | not_confirmed_on_holdout |
| Vegetables MoM, full nowcast | 228 | 160 | 68 | +1.5% | +6.2% | −0.047 | 0.257 | not_confirmed_on_holdout |
| Vegetables MoM, driver-only | 228 | 160 | 68 | −4.9% | −2.9% | −0.019 | 0.119 | not_confirmed_on_holdout |
| Sugar MoM, full nowcast | 228 | 160 | 68 | −6.7% | −0.1% | −0.066 | 0.994 | not_confirmed_on_holdout |
| Sugar MoM, driver-only | 228 | 160 | 68 | −43.5% | −82.1% | 0.385 | 0.0218 | not_confirmed_on_holdout |

**Twenty-two of twenty-three do not survive.** Only the fuel efficiency edge is `confirmed_on_holdout`; it is still reported as exploratory rather than a finding, for the reasons given in §5.2.1 (that it was never predeclared) and in §5.8 (its Bonferroni-adjusted p is 0.68 and its BH q-value 0.23 over this same 23-test family, so it survives neither correction — `multiple_testing.json`). Every other target's holdout skill either collapses toward zero or reverses sign, so none of the twenty-two changes the verdict already reported for it in §5.1–§5.8: the map remains uniformly null in the confirmatory sense.

**Two rows read `not_confirmed_on_holdout` while showing a DM p below 0.05, and the reason is the direction of the test.** Dairy & eggs driver-only (p = 0.0201) and sugar driver-only (p = 0.0218) have holdout skills of −35.0% and −82.1%: the selected model is significantly *worse* than the naive baseline it was scored against, not better. `confirmed_on_holdout` requires positive skill, `p < 0.05` and `dm_stat < 0` jointly, so a significant loss correctly fails it. This is the same asymmetry §4.9 records as `direction` in the multiplicity artifact, and it is why that artifact reports one nominal positive rather than three.

**The clearest single illustration of the bias this protocol exists to catch is YoY inflation.** Its selection-stage skill, +20.5%, is the largest of any target across all twenty-three, selected or not — and it fully evaporates on the holdout (+4.3%, DM p = 0.71). A verdict built only from the selection segment would have reported a strong edge; the same model on data selection never touched shows almost none. Sugar driver-only is the same mechanism at its most violent: −43.5% in selection becomes −82.1% on the holdout.

**Electricity's driver-only row — the withdrawn +28.3% flagship of §5.7 — shows the smallest shrinkage of the nine targets pre-registered 2026-08-08** (−0.030, i.e. the holdout skill is marginally less negative than selection). That is consistent with, not in tension with, §4.7's account of what the +28.3% figure actually was: both stages sit at essentially zero because there was never a real edge to shrink away from, only a mean-reverting target measured against the wrong baseline.

Frame sizes at run time matched each pre-registration's feasibility check exactly (105/104/214/227 rows for the four frame shapes newly run in 2026-08-08; 228 for all six sub-category frames). The smallest holdout segment across all twenty-three is fuel's 29 predictions, 2.4× `MIN_HOLDOUT_PREDICTIONS`; every other row has at least 31, and the twelve sub-category rows all sit at 68 (5.7×). None is reported as underpowered.
...
| Food MoM (n = 103) | no better than naive | no better than naive | no better than naive |
...
**Code.** The full analysis code and the frozen result artifacts are in the repository at `github.com/sindoussss/ph-economic-pressure-ai` (MIT license). The entire audit — every table, significance test, multiple-comparison correction, power analysis, and figure — regenerates with `python -m ph_economic_ai.benchmark.run`; the reported numbers were verified to reproduce byte-for-byte, most recently on 2026-08-20, when a fresh run rewrote only the artifacts’ `generated_at` timestamp and left every reported value and every figure identical. **No DOI is claimed for this work.** The repository’s existing `v1.0.0` and `v2.0.0` tags date from June 2026, predate the calendar correction of 2026-07-28, and share no sample size with the results reported here; they are not an archive of this manuscript and should not be cited as one. The state described here is identified by the frozen artifacts’ own content hash, `data_hash = 19979bdda7700d6a`.
...
| Method | Full nowcast RMSE (n = 204) | Driver-only RMSE (n = 204) | Driver-only, robust RMSE (n = 198) |
|---|---|---|---|
...
**B.7 Food-CPI MoM nowcast** (n = 204; best naive = random_walk). Full nowcast (own-lag + drivers) vs driver-only; driver-only edge is null at both windows.

| Method | Full nowcast RMSE | Driver-only RMSE (n = 204) | Driver-only, robust RMSE (n = 198) |
|---|---|---|---|
...
**B.8 Electricity-CPI MoM nowcast** (n = 204; best naive = **mean**). The driver-only edge is significant against the random walk at both windows and disappears against the mean, which is the whole of §5.7.
