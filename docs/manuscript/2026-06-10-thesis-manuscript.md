...
3. A **closed form for baseline-induced spurious skill** (§4.7). That the historical mean belongs in a naive pool is implicit in Hyndman and Koehler (2006) and in the Atkeson–Ohanian (2001) benchmark argument; what is supplied here is the magnitude. A forecaster carrying no information beyond the unconditional mean scores S(ρ) = 1 − [2(1 − ρ)]^(−1/2) against the random walk, positive exactly when the target's lag-1 autocorrelation ρ < ½, and ≈ +29% at ρ = 0. The expression matches simulation to ≤ 0.011 and predicts the observed gap on all five real targets to ≤ 0.022. Inverted, it is a diagnostic any reader can apply to a published edge. Applied to this audit's own flagship finding — an electricity "driver edge" of +28.3% that cleared significance, sub-sample stability, a real-time-vintage re-test, a Bonferroni correction, and an institutionally accurate mechanism — it returns an implied ρ of 0.027 against a measured 0.002: the result is fully explained without any driver.
...
That the mean belongs in a naive pool is not itself a new observation: it is implicit in the scale-free accuracy measures of Hyndman and Koehler (2006), and the Atkeson–Ohanian (2001) result is precisely an argument about which baseline an inflation forecast must clear. What this section contributes is a statement of **when the omission matters and by exactly how much**.
